# future.p2p: A Peer-to-Peer Compute Cluster in R ![](reference/figures/world-p2p-network-three-users.png)
...
that you trust, i.e. where you trust all the P2P users and the user who
...
running P2P workers in a sandboxed virtual machine (VM,
...
2.  have a *[pico.sh](https://pico.sh/) account*.

See the ‘Getting Started’ vignette for how to set this up, but the gist
for creating an SSH key pair if you already don’t have one is to:

``` sh
$ ssh-keygen
```

With the key pair create a [pico.sh](https://pico.sh/) account by
logging into their server:
...
username, and click ENTER. Finally, verify SSH access to `pipe.pico.sh`
(sic!);
...
example, this means ‘bob’, ‘carol’, ‘diana’, and ‘alice’ may use the P2P
cluster at the same time. Just like with any other future backend, we
use [`plan()`](https://future.futureverse.org/reference/plan.html) to
specify that we want to parallelize via the P2P cluster.
...
process any parallel tasks. This is where the peer-to-peer concept comes
in, where we contribute our idle compute cycles to the cluster for
others to make use of. To contribute your R compute power to the
...
the `pubkeys` menu, where you have options to add additional *public SSH
keys* of yours. This way, you can use your pico.sh account from multiple
computer systems, which can be handy if you want to set up parallel
workers on one system and harness their compute power from another.
...
it might stall forever. If that happens, press Ctrl-C to interrupt and
retry by disabling the proxy settings using:
...
If the latter works for you, launch R by unsetting environment variable
`http_proxy`, e.g.
